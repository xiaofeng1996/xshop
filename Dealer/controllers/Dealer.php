...
class Dealer extends MY_Controller {
	...
	public function index($offset="")
	{
		...
		//请求的URL地址
		$config['base_url']=site_url('dealer/index');
		//查询出所有的条数
		$config['total_rows']=$this->db->count_all('dea_goods');
		//设置每页显示的条数
		$config['per_page']=4;
		//传递的页码参数的值
		...
		//echo $where1;die;
		if($where1 == 1){
			$data['data'] = $this -> db -> where('is_delete',0)->limit($limit,$offset1) ->get('dea_goods')->result_array();
		}else{
			$data['data'] = $this -> db -> where($where)-> where('is_delete',0)->limit($limit,$offset1) ->get('dea_goods')->result_array();
		}
		...
		$this->load->view('admin/dealer/goods.html',$data);
	...
	public function add_goods(){
		...
		if($this->input->post()){
			...
			if($this->upload->do_upload('goods_img')){
				...
				$res = $this->db->insert('dea_goods', $data);
				...
				if(!empty($attr_name) & !empty($attr_id)){
					...
					foreach ($data1 as $v){
						$this->db->insert('dea_goods_attr', $v);
					}
					if($res > 0){
						echo "<script>alert('添加成功');location.href='".site_url('dealer/index')."'</script>";
					}
				}
			...
			$this->load->view('admin/dealer/add_goods.html',$datas);
		}
	}
