...
class Login extends CI_Controller {
	...
	public function check_login(){
		...
		$data = $this->db->where('adminname',$account)->where('pwd',$password)->get('dea_user')->result_array();
		...
		if($account==$data[0]['adminname']&& $password==$data[0]['pwd']){
			...
			$this->session->set_userdata('dea_admin_name',$data[0]['adminname']);
	...
	public function login_out(){
		$this->session->unset_userdata('dea_admin_name');
